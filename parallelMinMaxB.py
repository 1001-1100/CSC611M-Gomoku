...
def parallelB(state, turn, depth):
    class workerThread(multiprocessing.Process):
        def __init__(self, state, mx, my, score, l, move):
            multiprocessing.Process.__init__(self)
            self.state = state
            self.move = move
            self.mx = mx
        ...
        def run(self):
            ...
            minMaxScore = score
            # If current move has a better score than the best one, change
            l.acquire()
            if(turn == blackTurn):
                if(minMaxScore >= gScore.value):
                    gScore.value = minMaxScore 
                    mx.value = move[0]
                    my.value = move[1]
            elif(turn == whiteTurn):
                if(minMaxScore <= gScore.value):
    ...
    count = 0
    # Shared values
    mx = multiprocessing.Value('d', 0)
    my = multiprocessing.Value('d', 0)
    ...
    for move in moves:
        ...
        worker = workerThread(np.copy(state), mx, my, score, l, move)
        count += 1
        worker.start()
    ...
    # Check shared values for the best move
    l.acquire()
    minMaxScore = score.value
    minMaxMove = (int(mx.value), int(my.value))
    l.release()
...
def gameMove(turn, player):
    ...
    data = parallelB(state, turn, depth)
    move = data[0]
...
with open('ParallelB.csv', 'a') as csvfile:
